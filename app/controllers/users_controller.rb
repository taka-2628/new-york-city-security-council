class UsersController < ApplicationController
  ...
  def index 
    users = User.all
    render json: users, include: ['neighborhood', 'cameras', 'social_media_platforms']
  end

  def show
    user = User.find_by(id: session[:user_id])
    if user
      render json: user, status: :ok, include: ['neighborhood', 'cameras', 'social_media_platforms']
    else
  ...
  def create
    ...
    if user.valid?
      ...
      render json: user, status: :created, include: ['neighborhood', 'cameras', 'social_media_platforms']
    else
